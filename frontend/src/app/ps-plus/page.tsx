...
export default function PsPlusPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_14%_0%,rgba(34,211,238,0.2),transparent_34%),radial-gradient(circle_at_86%_8%,rgba(217,70,239,0.18),transparent_34%),linear-gradient(180deg,#06070b_0%,#0b1020_45%,#06080f_100%)] text-zinc-50">
      ...
      >
        <motion.div
          className="absolute -top-32 left-10 h-64 w-64 rounded-full bg-cyan-500/20 blur-3xl"
          animate={{ y: [0, 20, -10, 0], opacity: [0.5, 0.8, 0.4, 0.5] }}
          transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute top-10 right-0 h-72 w-72 rounded-full bg-fuchsia-500/20 blur-3xl"
          animate={{ y: [0, -25, 10, 0], opacity: [0.4, 0.7, 0.3, 0.4] }}
          transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-[-6rem] left-1/2 h-80 w-80 -translate-x-1/2 rounded-full bg-violet-400/18 blur-3xl"
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col px-4 py-10 sm:px-8 lg:px-12">
        <header className="mb-10 space-y-4">
          <p className="inline-flex items-center gap-2 rounded-full border border-cyan-300/35 bg-cyan-500/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.32em] text-cyan-100/90">
          ...
          <motion.h1
            ...
            className="max-w-4xl mh-gradient-text text-3xl font-extrabold tracking-tight sm:text-4xl lg:text-5xl"
                ...
                >
                  {active && (
                    <motion.span
                      ...
                      className="absolute inset-0 rounded-full bg-gradient-to-r from-cyan-500 via-violet-500 to-fuchsia-500 shadow-[0_0_18px_rgba(34,211,238,0.7)]"
                      transition={{
                        type: "spring",
                        stiffness: 320,
                        damping: 26,
        ...
        <section className="mb-8 grid gap-4 sm:grid-cols-3">
          {PLANS.map((plan) => {
            ...
            const baseClasses =
              "mh-organic-card relative flex h-full flex-col justify-between overflow-hidden rounded-3xl border p-4 text-xs shadow-[0_0_35px_rgba(0,0,0,0.65)] origin-center backdrop-blur-xl";

            const colorsByPlan: Record<PlanId, string> = {
              ESSENTIAL:
                "border-cyan-300/35 bg-[linear-gradient(165deg,rgba(12,22,38,0.9),rgba(8,14,28,0.78))] text-zinc-100",
              EXTRA:
                "border-violet-300/35 bg-[linear-gradient(165deg,rgba(24,14,44,0.86),rgba(12,10,30,0.78))] text-zinc-100",
              DELUXE:
                "border-fuchsia-300/35 bg-[linear-gradient(165deg,rgba(36,12,36,0.86),rgba(15,10,26,0.8))] text-zinc-100",
            };
              ...
              >
                ...
                <div className="relative z-10 space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {plan.id === "DELUXE" && (
                      <span className="rounded-full bg-fuchsia-400/15 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-fuchsia-200">
                        Premium
                      </span>
                    )}
                  </div>
                    ...
                    {benefits.map((b) => (
                      ...
                      >
                        <span
                          className={
                            plan.id === "ESSENTIAL"
                              ? "mt-0.5 h-1.5 w-1.5 rounded-full bg-cyan-400"
                              : plan.id === "EXTRA"
                              ? "mt-0.5 h-1.5 w-1.5 rounded-full bg-violet-400"
                              : "mt-0.5 h-1.5 w-1.5 rounded-full bg-fuchsia-400"
                          }
                        />
                        <span
                          className={
                ...
                <div className="relative z-10 mt-4">
                  <motion.button
                    ...
                    onPointerDown={(e) => triggerRipple(e)}
                    className={`mh-pressable mh-focus w-full rounded-2xl px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.18em] shadow-[0_0_20px_rgba(0,0,0,0.35)] ${
                      plan.id === "ESSENTIAL"
                        ? "bg-gradient-to-r from-cyan-500 to-cyan-400 text-black hover:from-cyan-400 hover:to-cyan-300"
                        : plan.id === "EXTRA"
                        ? "bg-gradient-to-r from-violet-500 to-fuchsia-400 text-white hover:from-violet-400 hover:to-fuchsia-300"
                        : "bg-gradient-to-r from-fuchsia-500 via-violet-400 to-cyan-400 text-black hover:from-fuchsia-400 hover:via-violet-300 hover:to-cyan-300"
                    }`}
        ...
        <section className="mb-10">
          <div className="overflow-x-auto">
            <div className="mh-panel mh-glass min-w-[720px] rounded-3xl p-4 shadow-[0_0_45px_rgba(0,0,0,0.45)] backdrop-blur-xl">
                  ...
                  {PLANS.map((plan) => (
                    ...
                    >
                      <motion.button
                        ...
                        onClick={() => handleAdd(plan)}
                        onPointerDown={(e) => triggerRipple(e)}
                        className="mh-pressable mh-focus inline-flex items-center justify-center rounded-full bg-gradient-to-r from-blue-600 via-indigo-500 to-sky-500 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-zinc-50 shadow-[0_0_20px_rgba(37,99,235,0.7)] hover:from-blue-500 hover:via-indigo-400 hover:to-sky-400"
